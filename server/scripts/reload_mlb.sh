#!/usr/bin/env bash
# Clear and reload a season of MLB data.
#
#   ./scripts/reload_mlb.sh 2026
#
# Needed when the *selection* logic in sources/mlb_source.py changes, not just
...
#
# Scoped to sport='mlb' throughout. NBA rows are counted before and after so a
...
Usage: $0 <season> <ENV_VAR_HOLDING_THE_URL>
  ...
  $0 2026 MIGRATION_TEST_DATABASE_URL   # Neon test branch
  $0 2026 DATABASE_URL                  # production

Naming the variable is required so that the database you are about to
delete MLB rows from is a decision, not a default.
USAGE
  exit 1
fi
...
read -r -p "Replace ALL sport='mlb' rows in \$${DB_VAR} with season $SEASON? [y/N] " ok
...
DATABASE_URL="$psql_url" python fetch_mlb_stats.py --season "$SEASON" --replace
cd ..
...
echo
echo "NBA and NFL counts above must be unchanged from 'Before'."
